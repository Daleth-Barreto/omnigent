...
def test_telegram_config_defaults(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:  # noqa: ARG001
...
def test_build_elicitation_keyboard_binary(
    ...
    elicit = ElicitationRequest(
        elicitation_id="elicit_101", message="Approve?", session_id="sess_1", questions=[]
    )
...
def test_build_elicitation_keyboard_form(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    elicit = ElicitationRequest(
        elicitation_id="elicit_202", message="Choose", session_id="sess_1", questions=[q1]
    )
